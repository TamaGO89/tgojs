...
function load_worksheet(result={},...filename) {
    const worksheet = xlsx.readFileSync(path.join(...filename));
...
function load_json(...filename) {
    return JSON.parse(readFileSync(path.join(...filename),"utf8"))
}
...
function _to_schema(variable) { return `${variable.type} ${variable.name.replaceAll(" ","_")}=${variable.tag};`; }
...
function parse_converts(converts) {
	...
	for (const conv of converts) {
		conv.cmd = new Function("remote","local",conv.formula
                .replaceAll("X",`remote.${conv.remote_variable}`)
                .replaceAll("Y",`local.${conv.local_variable}`));
	}
}
